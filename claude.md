...
| K | ATR SL experiment for SYN/LINK | **SOL/CAD PROMOTED to ACTIVE 2026-08-25** — see "Live Symbol Universe" above. **SYN/USD groundwork done the same day (NOT promoted)** — fresh walk-forward on current strategy hash still HOLDS (PF 1.75 train/validation, stronger than SOL's), but needs its own new capital ($250-$690 depending on volatility, on top of what BTC+SOL already committed), a real FX-conversion step (SYN/USD is genuinely USD-quoted — confirmed Kraken `USD/CAD` conversion cost 0.20%/leg), and its 24h liquidity ($49,371) is sitting right at the $50k gate, not comfortably above it like SOL/BTC — re-check before acting. Full detail: `.memory/decisions/multi-symbol-validation.md`. |
