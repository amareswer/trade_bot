...
Run: `python -m pytest --tb=short -q` — must show **719 passed**. (This line had drifted to a
...
Still open (VPS-readiness, not built): a proactive scheduled health digest (so you don't have
to remember `/status_crypto`), a generic stuck-loop detector, and remote-reachable dashboards
(currently local HTML files). See the "Current operational status" note.
