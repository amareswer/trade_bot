...
`.memory/decisions/mean-reversion-experiment-2026-08-28.md`, CLAUDE_HISTORY.md. Then 790→796,
+6 for the dead-mover prune (`tests/stock/test_universe_refresh.py` 12→18) — `stock_bot.main._prune_dead_movers`
drops a top-mover that returns no price data for `_MOVER_DEAD_AFTER`=3 consecutive cycles
(BLX.TO 2026-08-28: only ~7 daily rows on Yahoo → `fetch_candles` None → a yfinance ERROR every
scan cycle); watchlist + held-position symbols are exempt, the trimmed list is re-persisted, and
`_dead_movers` clears on the daily refresh so a symbol can return.
...
Run: `python -m pytest --tb=short -q` — must show **796 passed**. (This line had drifted to a
...
dead_counts, exempt)` (pure helper, `stock_bot/main.py`): a mover with `price_data[sym] is
None` for `_MOVER_DEAD_AFTER`=3 consecutive cycles is dropped from `top_movers` for the rest
of the session, the trimmed list re-persisted, `all_symbols` rebuilt. Watchlist symbols and
held positions are `exempt` (their fetch failures are real signal, not noise, and a held
position must always stay in the scan). `_dead_movers` (session-local dict) clears on the
daily universe refresh, so a symbol that recovers is given a fresh chance. Called once per
cycle right after `active_symbols` is computed. Tests: `tests/stock/test_universe_refresh.py`
12→18. Suite 790→796. No strategy files touched.
