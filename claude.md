...
Run: `python -m pytest --tb=short -q` — must show **731 passed**. (This line had drifted to a
...
**Debounced same day (2026-08-27)** — first live run flapped: BNS and GM sit right on the
`PAPER_MAX_EXPOSURE_PCT=0.25` ceiling (one ~14% position + `PAPER_RISK_PCT=0.20` per trade
means a 2nd full-size BUY rarely fits), and their rule signal toggled BUY↔HOLD every other
cycle — each toggle changed the set and re-alerted. Now `state['alerted']` tracks each
