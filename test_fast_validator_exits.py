...
def _make_validator(positions: list[FastPosition]) -> FastValidator:
    """Validator with tmp CSV path — never touches the real fast_trades.csv."""
    tmp_csv = os.path.join(tempfile.mkdtemp(), "fast_trades.csv")
...
def test_max_hold_exits_via_live_price_when_no_candles():
    """Held past max_hold + no candles this cycle → MAX_HOLD exit at live price."""
    v = _make_validator([_pos("AMZN", held_hours=50.0)])
...
def test_max_hold_no_candles_and_no_live_price_keeps_position():
...
def test_no_candles_under_max_hold_keeps_position_without_price_call():
    """Under max hold with no candles: SL/TP wait for data — no live-price call."""
    v = _make_validator([_pos("HOOD", held_hours=2.0)])
    with v._tmp_csv_patch, \
...
def test_stop_loss_fires_with_candles_present():
    """Regression: normal SL path unchanged by the no-candles fallback."""
    v = _make_validator([_pos("HOOD", held_hours=2.0, entry=100.0)])
...
def test_corrupt_close_skips_exit_check():
    ...
    v = _make_validator([_pos("META", held_hours=2.0, entry=560.0)])
...
def test_sane_gap_within_threshold_still_exits():
