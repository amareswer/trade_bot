...
| 2026-06-25 | AC.TO SL/TP watcher unprotected | `get_live_price()` via `fast_info` had no corruption guard — a $61 fake price on AC.TO (real ~$17) would fire TP at $27.20 threshold and close the position at a fake profit. The main scan loop guard in `fetch_candles()` does not protect the 30s daemon watcher; both code paths are independent. | Added 20% previous-close deviation check to `get_live_price()` in `intraday_price.py`. Uses `getattr(info, "previous_close", None)` on the already-fetched `fast_info` object — zero extra network calls. Returns `None` on corruption; `_check_open_positions_sl_tp()` already handles `None` with `continue`. |
| 2026-06-25 | Duplicate positions in Portfolio Overview | `build_summary()` and `build_paper_summary()` both read `self._positions` on the same executor — renderer appended both lists → every position showed twice (REAL + PAPER badge). | Added guard in `renderer.py` `_portfolio_overview_html()`: REAL section skipped when `paper_summary` has active positions. |
| 2026-06-25 | yfinance rate limiting | `ThreadPoolExecutor(max_workers=10)` fired all 16 symbol downloads simultaneously. Yahoo Finance returned empty DataFrames for some symbols in the burst, showing as "no data" in the scan log. No sleep existed anywhere in `fetch_candles()`. | Added `_yf_download_lock = threading.Lock()` to serialize all `yf.download()` calls in `price_feed.py`. `time.sleep(0.5)` runs in the `finally` block (fires on success and exception, lock still held). 16 symbols → ~8s total fetch time, well within the 120s loop interval. |
